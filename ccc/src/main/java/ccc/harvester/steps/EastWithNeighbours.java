...
public class EastWithNeighbours extends HarvestStep {
	...
	public List<Cell> doIt(CornField field, Cell startCell) {
		...
		switch (getDirection()) {
		case NORTH:
			cellsEastFromHereWithNeighbours = field.cellsEastFromHereNorthNeighbours(getMowers(), isNeighboursFirst(),
					getCount(), row, column);
			break;
		case SOUTH:
			cellsEastFromHereWithNeighbours = field.cellsEastFromHereSouthNeighbours(getMowers(), isNeighboursFirst(),
					getCount(), row, column);
			break;
		case EAST:
		case WEST:
